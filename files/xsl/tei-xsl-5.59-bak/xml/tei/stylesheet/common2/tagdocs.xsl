...
<xsl:stylesheet xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:s="http://www.ascc.net/xml/schematron" xmlns:a="http://relaxng.org/ns/compatibility/annotations/1.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:="http://www.-c.org/ns/1.0" xmlns:x="http://www.-c.org/ns/Examples" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="fo s a  html rng x xs sch" version="1.0">
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet" type="stylesheet">
    <desc>
      <p>  stylesheet for weaving  ODD documents</p>
    ...
    02111-1307 USA </p>
      ...
      <p>Id: $Id: tagdocs.xsl 9514 2011-10-17 09:13:49Z rahtz $</p>
  ...
  <xsl:template match="ptr|ref" mode="weave">
    <xsl:choose>
      <xsl:when test="ancestor::remarks or ancestor::listRef or ancestor::valDesc">
        <xsl:choose>
          <xsl:when test="starts-with(@target,'#') and id(substring(@target,2))">
            <xsl:call-template name="makeInternalLink">
              <xsl:with-param name="target" select="substring(@target,2)"/>
              <xsl:with-param name="ptr" select="if (self::ptr)
		  ...
		  <xsl:with-param name="ptr" select="if (self::ptr)
						 then true() else false()"/>
                  <xsl:with-param name="dest">
                    <xsl:text>http://www.-c.org/release/doc/-p5-doc/</xsl:text>
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>Process element attDef/datatype</desc>
  </doc>
...
-->
        ...
        <xsl:element namespace="{$outputNS}" name="{$rowName}">
          ...
          <xsl:element namespace="{$outputNS}" name="{$cellName}">
            ...
            <xsl:call-template name="generandirectParents"/>
          </xsl:element>
        </xsl:element>
        <xsl:element namespace="{$outputNS}" name="{$rowName}">
          <xsl:element namespace="{$outputNS}" name="{$cellName}">
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>Process element elementSpec/content</desc>
  </doc>
  <xsl:template match="elementSpec/content" mode="weave">
    ...
    <xsl:element namespace="{$outputNS}" name="{$rowName}">
      ...
      <xsl:element namespace="{$outputNS}" name="{$cellName}">
        ...
        <xsl:for-each select="valList[@type='closed']">
          <xsl:call-template name="i18n">
  ...
  <xsl:template match="constraintSpec[parent::elementSpec or parent::attDef]" mode="weave">
    <xsl:element namespace="{$outputNS}" name="{$rowName}">
      <xsl:element namespace="{$outputNS}" name="{$cellName}">
        <xsl:attribute name="{$rendName}">
          <xsl:text>wovenodd-col1</xsl:text>
      ...
      <xsl:otherwise>
        <xsl:if test="attList//attDef">
          <xsl:element namespace="{$outputNS}" name="{$tableName}">
            ...
            <xsl:apply-templates mode="summary" select="attList//attDef"/>
          </xsl:element>
        </xsl:if>
        <xsl:call-template name="showAttClasses">
          <xsl:with-param name="minimal">true</xsl:with-param>
  ...
  <xsl:template match="exemplum" mode="doc">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not(../exemplum[@xml:lang=$documentationLanguage])                    and (@xml:lang='en'      or @xml:lang='und'      or @xml:lang='mul')">
        <xsl:call-template name="showExample"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>Process element macroSpec/content</desc>
  </doc>
  ...
  <xsl:template match="valDesc" mode="weave">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not(@xml:lang) and         not($documentationLanguage='en')          and         ../valDesc[@xml:lang=$documentationLanguage]">
      </xsl:when>
      <xsl:otherwise>
        <xsl:element namespace="{$outputNS}" name="{$rowName}">
          <xsl:element namespace="{$outputNS}" name="{$cellName}">
  ...
  <xsl:template name="generateModelParents">
    ...
    <xsl:element namespace="{$outputNS}" name="{$divName}">
      ...
      <xsl:variable name="Parents">
        <xsl:for-each select="key('REFS',@ident)/ancestor::content/parent::*">
  ...
  <xsl:template name="generandirectParents">
    <xsl:element namespace="{$outputNS}" name="{$divName}">
      <xsl:attribute name="{$rendName}">parent</xsl:attribute>
      <xsl:variable name="here" select="."/>
	...
	<xsl:for-each select="key('ATTREFS-ELEMENT',$this)">
	  ...
	  <Item>
		...
		<xsl:call-template name="linkTogether">
	      ...
	      <xsl:text>/@</xsl:text>
	      <xsl:for-each select="ancestor::attDef">
		<xsl:value-of select="(altIdent|@ident)[last()]"/>
	      </xsl:for-each>
	    </xsl:element>
	  </Item>
	</xsl:for-each>
  ...
  <xsl:template match="exemplum" mode="weave">
    <xsl:if test="x:egXML/* or x:egXML/text() or text()">
      <xsl:apply-templates select="." mode="doc"/>
    </xsl:if>
  </xsl:template>
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>No-op processing of elements gloss and desc in
  ...
  <xsl:template name="showAttClasses">
    ...
    <xsl:variable name="clatts">
      <xsl:for-each select="ancestor-or-self::elementSpec|ancestor-or-self::classSpec">
    ...
    <xsl:choose>
            ...
            <xsl:call-template name="i18n">
              <xsl:with-param name="word">
                <xsl:choose>
                  ...
                  <xsl:when test=".//attDef">In addition to global attributes and those inherited from</xsl:when>
      ...
      <xsl:when test="ancestor::schemaSpec and not(key('CLASSES','att.global'))">
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="i18n">
          <xsl:with-param name="word">
            <xsl:choose>
              <xsl:when test="not($autoGlobal='true')">Attributes</xsl:when>
              <xsl:when test=".//attDef">In addition to global attributes</xsl:when>
  ...
  <xsl:template name="processSpecDesc">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not($localsource='')">
        <xsl:for-each select="document($localsource)/">
  ...
  <xsl:template name="makeTagsetInfo">
    ...
    <xsl:for-each select="key('MODULES',@module)/ancestor::div[last()]">
      <xsl:text> — </xsl:text>
      <xsl:call-template name="makeInternalLink">
  ...
  <xsl:template name="generateChildren">
    ...
    <xsl:choose>
      <xsl:when test="content//rng:ref[@name='macro.anyXML']">
        <xsl:element namespace="{$outputNS}" name="{$segName}">
          <xsl:attribute name="xml:lang">
            <xsl:value-of select="$documentationLanguage"/>
          </xsl:attribute>
