...
<xsl:stylesheet xmlns:a="http://relaxng.org/ns/compatibility/annotations/1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:i="http://www.iso.org/ns/1.0" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:s="http://www.ascc.net/xml/schematron" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:="http://www.-c.org/ns/1.0" xmlns:x="http://www.-c.org/ns/Examples" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://www.w3.org/1998/Math/MathML" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dbk="http://docbook.org/ns/docbook" exclude-result-prefixes="a fo html i rng s sch  x xi xs xsl         m atom xlink xhtml dbk" version="1.0">
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet" type="stylesheet">
    <desc>
      ...
      <p>Id: $Id: verbatim.xsl 9513 2011-10-17 08:55:51Z rahtz $</p>
      <p>Copyright: 2011,  Consortium</p>
    </desc>
  </doc>
  <xsl:strip-space elements="x:* rng:* xsl:* xhtml:* atom:* m:*"/>
  ...
  <xsl:param name="omitNSDecls">http://www.-c.org/ns/1.0</xsl:param>
  ...
  <xsl:template name="verbatim-getNamespacePrefix">
    ...
    <xsl:variable name="prefix" select="getPrefix($ns,ancestor-or-self::*[1])[1]"/>
    <xsl:choose>
      ...
      <xsl:when test="$ns='http://www.-c.org/ns/geneticEditions'">ge</xsl:when>
    </xsl:choose>
  </xsl:template>
...
-->
  ...
  <xsl:template match="*" mode="verbatim">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not(parent::*)  or parent::x:egXML">
        <xsl:choose>
    ...
    <xsl:if test="$showNamespaceDecls='true' or parent::x:egXML[@rend='full']">
      <xsl:choose>
        <xsl:when test="not(parent::*)">
          <xsl:call-template name="nsList"/>
        </xsl:when>
        <xsl:when test="parent::x:egXML and not(preceding-sibling::*)">
          <xsl:call-template name="nsList"/>
        </xsl:when>
      </xsl:choose>
    </xsl:if>
  ...
  <xsl:template name="verbatim-makeElementName">
    ...
    <xsl:choose>
      <xsl:when test="namespace-uri()='http://www.-c.org/ns/Examples'">
  ...
  <xsl:template name="verbatim-makeIndent">
    <xsl:variable name="depth" select="count(ancestor::*[not(namespace-uri()='http://www.-c.org/ns/1.0')])"/>
  ...
  <xsl:template match="@*" mode="verbatim">
    ...
    <xsl:if test="count(../@*)&gt;$attsOnSameLine or      string-length($L)&gt;$attLength or     ancestor::cell[not(@rend='wovenodd-col2')] or     namespace-uri()='http://www.w3.org/2005/11/its' or     string-length(.)+string-length(name(.)) &gt;     $attLength">
      <xsl:call-template name="verbatim-lineBreak">
        <xsl:with-param name="id">5</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="verbatim-makeIndent"/>
  ...
  <xsl:template match="*" mode="ns">
    <xsl:for-each select="namespace::*">
      ...
      <xsl:choose>
        ...
        <xsl:when test=".='http://www.-c.org/ns/Examples'"/>
  ...
  <xsl:function name="getPrefix" as="xs:string*">
    <xsl:param name="ns"/>
    <xsl:param name="here"/>
