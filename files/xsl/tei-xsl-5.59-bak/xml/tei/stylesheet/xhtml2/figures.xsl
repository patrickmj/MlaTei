...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" 
  ...
  <xsl:template match="cell">
    <xsl:variable name="cellname">
      <xsl:choose>
	<xsl:when test="parent::row[@rend='thead']">th</xsl:when>
  ...
  <xsl:template match="figure">
    ...
    <xsl:choose>
      <xsl:when test="parent::head or @rend='inline' or @place='inline'">
	<xsl:apply-templates/>
      </xsl:when>
	...
	<xsl:element name="{$container}">
	  ...
	  <xsl:if test="head">
	    <xsl:variable name="caption">
	      <xsl:choose>
		<xsl:when test="ancestor::front and  $numberFrontFigures='true'">
		  <xsl:call-template name="i18n">
		    <xsl:with-param name="word">figureWord</xsl:with-param>
		  </xsl:call-template>
		  <xsl:text> </xsl:text>
		  <xsl:number count="figure[head]" from="front" level="any"/>
		  <xsl:text>. </xsl:text>
		</xsl:when>
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>Process element figure/head</desc>
  </doc>
  ...
  <xsl:template match="table">
	 ...
	 <xsl:if test="@xml:id">	   
	   <xsl:call-template name="makeAnchor"/>
	    ...
	    <xsl:choose>
	      <xsl:when test="row[@rend='thead']">
		<thead>
		  <xsl:apply-templates
		      select="row[@rend='thead']"/>
		</thead>
		<tbody>
		  <xsl:apply-templates select="row[not(@rend='thead')]"/>
		</tbody>
	      </xsl:when>
	      <xsl:otherwise>
	  ...
	  <xsl:choose>
	    ...
	    <xsl:when test="starts-with(@mimeType, 'video')">
	      <video src="{$graphicsPrefix}{$File}"
		     controls="controls">
		<xsl:if test="../graphic[@type='thumbnail']">
		  <xsl:attribute name="poster">
		    <xsl:value-of select="../graphic[@type='thumbnail']/@url"/>
		  </xsl:attribute>
		</xsl:if>
	      </video>
	    </xsl:when>
