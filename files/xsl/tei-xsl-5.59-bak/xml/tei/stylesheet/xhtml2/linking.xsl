...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" 
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet" type="stylesheet">
      ...
      Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA </p>
         ...
         <p>Id: $Id: linking.xsl 9535 2011-10-19 21:08:49Z rahtz $</p>
  ...
  <xsl:template match="*" mode="ident">
      ...
      <xsl:choose>
         ...
         <xsl:when test="self::TEI and parent::teiCorpus">
	   <xsl:value-of select="$masterFile"/>
	   <xsl:call-template name="addCorpusID"/>
	 </xsl:when>
         <xsl:otherwise>
  ...
  <xsl:template match="*" mode="generateLink">
	...
	<xsl:when test="$filePerPage='true'">
	  ...
	  <xsl:value-of select="$standardSuffix"/>
         ...
         <xsl:when test="ancestor::elementSpec and not($STDOUT='true')">
	           <xsl:text>ref-</xsl:text>
	           <xsl:value-of select="ancestor::elementSpec/@ident"/>
            <xsl:value-of select="$standardSuffix"/>
	           <xsl:value-of select="concat($Hash,$ident)"/>
         </xsl:when>
         <xsl:when test="ancestor::classSpec and not($STDOUT='true')">
	           <xsl:text>ref-</xsl:text>
	 ...
	 <xsl:when test="self::text and $splitLevel=0">
	   <xsl:value-of select="$ident"/>
	   <xsl:value-of select="$standardSuffix"/>
	 </xsl:when>
  ...
  <xsl:template name="locateParentDiv">
	...
	<xsl:when
	     test="ancestor-or-self::body/parent::text/ancestor::group">
            <xsl:apply-templates mode="ident" select="ancestor::text[1]"/>
         </xsl:when>

	 <xsl:when test="ancestor-or-self::front/parent::text/ancestor::group">
            <xsl:apply-templates mode="ident" select="ancestor::text[1]"/>
         </xsl:when>

	 <xsl:when test="ancestor-or-self::back/parent::text/ancestor::group">
            <xsl:apply-templates mode="ident" select="ancestor::text[1]"/>
         </xsl:when>

         <xsl:when test="ancestor-or-self::div and number($splitLevel) &lt; 0">
            <xsl:apply-templates mode="ident" select="ancestor::div[last()]"/>
         </xsl:when>

         <xsl:when test="ancestor-or-self::div">
	   <xsl:variable name="ancestors" select="count(ancestor-or-self::div)"/>
	   <xsl:variable name="diff" select="$ancestors - number($splitLevel)"/>
	   <xsl:variable name="what" select="if ($diff &lt;= 1) then 1
					     else $diff "/>
	   <xsl:apply-templates mode="ident" select="ancestor-or-self::div[$what]"/>
         </xsl:when>

         <xsl:otherwise>
	   <xsl:variable name="ancestors" select="count(ancestor-or-self::*[local-name()='div1'
				 or local-name()='div2'
				 or local-name()='div3'
				 or local-name()='div4'
				 or local-name()='div5'
	   ...
	   <xsl:variable name="what"
            ...
            <xsl:apply-templates mode="ident"
				 select="ancestor-or-self::*[local-name()='div1'
				 or local-name()='div2'
				 or local-name()='div3'
				 or local-name()='div4'
				 or local-name()='div5'
      ...
      <a>
        <xsl:choose>
            ...
            <xsl:when test="parent::item/parent::list[@rend]">
	              <xsl:attribute name="class">
	                 <xsl:value-of select="parent::item/parent::list/@rend"/>
	              </xsl:attribute>
            </xsl:when>
      ...
      <xsl:choose>
	   ...
	   <xsl:choose>
		 ...
		 <xsl:for-each select="id($W)">
			 ...
			 <xsl:attribute name="title">
			   <xsl:value-of
			       select="translate(normalize-space(head[1]),'&gt;&lt;','')"/>
			 </xsl:attribute>
		       </xsl:when>
		     </xsl:choose>
		 </xsl:for-each>
