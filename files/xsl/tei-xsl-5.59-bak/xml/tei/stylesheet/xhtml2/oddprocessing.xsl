...
<xsl:stylesheet version="1.0"
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet" type="stylesheet">
    <desc>
      ...
      <p>Id: $Id: oddprocessing.xsl 9585 2011-11-02 22:48:48Z rahtz $</p>
  ...
  <xsl:key name="MODEL-CLASS-MODULE" match="classSpec[@type='model']" use="@module"/>
  <xsl:key name="ATT-CLASS-MODULE" match="classSpec[@type='atts']" use="@module"/>
  <xsl:key name="ELEMENT-MODULE" match="elementSpec" use="@module"/>
  <xsl:key name="MACRO-MODULE" match="macroSpec" use="@module"/>
  <xsl:key name="ELEMENT-ALPHA" match="elementSpec" use="substring(translate(@ident,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),1,1)"/>
  <xsl:key name="MODEL-CLASS-ALPHA" match="classSpec[@type='model']" use="substring(translate(@ident,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),7,1)"/>
  <xsl:key name="ATT-CLASS-ALPHA" match="classSpec[@type='atts']" use="substring(translate(@ident,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),5,1)"/>
  ...
  <xsl:template name="refdoc">
    ...
    <xsl:choose>
      <xsl:when test="self::classSpec and not(@ident='att.global') and         count(key('CLASSMEMBERS',@ident))=0">
        <xsl:if test="$verbose='true'">
  ...
  <xsl:template match="schemaSpec">
    <xsl:choose>
      <xsl:when test="specGrpRef">
        ...
        <xsl:for-each select="$SPECS/schemaSpec">
  ...
  <xsl:template name="schemaSpecWeave">
    ...
    <xsl:choose>
      <xsl:when test="$summaryDoc='true'">
        ...
        <xsl:for-each select="classSpec[@mode or @rend='change']        | macroSpec[(@mode or @rend='change')]        | elementSpec[(@mode or @rend='change')]">
          <xsl:sort select="@ident"/>
          <xsl:apply-templates mode="weave" select="."/>
        </xsl:for-each>
        <h2>Schema <xsl:value-of select="@ident"/>:  unchanged  components</h2>
        <table>
          <xsl:for-each select="classSpec[not(@mode or @rend)]          | macroSpec[not(@mode or  @rend)]          | elementSpec[not(@mode or @rend)]">
            <xsl:sort select="@ident"/>
            <tr>
  ...
  <xsl:template match="gi">
    ...
    <xsl:choose>
      <xsl:when test="parent::ref or parent::head or string-length(@scheme)&gt;0">
        <span class="gi">
          <xsl:text>&lt;</xsl:text>
          <xsl:apply-templates/>
          <xsl:text>&gt;</xsl:text>
  ...
  <xsl:template match="ptr[@type='cit']">
    <a class="citlink">
  ...
  <xsl:template match="divGen[@type='macrocat']" priority="100">
  ...
  <xsl:template match="divGen[@type='elementcat']"  priority="100">
  ...
  <xsl:template match="divGen[@type='modelclasscat']"  priority="100">
    <div class="atozwrapper">
      <xsl:call-template name="atozHeader">
  ...
  <xsl:template match="divGen[@type='attclasscat']"  priority="100">
    <div class="atozwrapper">
      <xsl:call-template name="atozHeader">
