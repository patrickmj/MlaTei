...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"  xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet" type="stylesheet">
      ...
      Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA xs </p>
         ...
         <p>Id: $Id: textstructure.xsl 9532 2011-10-19 15:52:16Z rahtz $</p>
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
   ...
   <xsl:template name="processTEI">
     <xsl:choose>
       ...
       <xsl:when test="($pageLayout = 'CSS' or $pageLayout = 'Table') and (TEI or teiCorpus)">
	 ...
	 <xsl:apply-templates/>
       ...
       <xsl:when test="TEI or teiCorpus and number($splitLevel)&gt;-1">
	 <xsl:if test="$verbose='true'">
  ...
  <xsl:template match="TEI" mode="split">
      ...
      <xsl:variable name="outName">
         <xsl:call-template name="outputChunkName">
	           <xsl:with-param name="ident">
	              <xsl:choose>
	                 <xsl:when test="parent::teiCorpus">
  ...
  <xsl:template match="front|body|back" mode="split">
      <xsl:for-each select="*">
         <xsl:choose>
            <xsl:when test="starts-with(local-name(.),'div')">
  ...
  <xsl:template match="divGen[@type='actions']">
      ...
      <dl>
         <xsl:for-each select="/TEI/text//note[@type='action']">
            <dt>
               <b>
  ...
  <xsl:template match="divGen[@type='index']">
	...
	<xsl:for-each select="key('INDEX',1)">
	  <REF>
	    <SORT>
	      <xsl:value-of select="lower-case(normalize-unicode(term,'NFD'))"/>
	    </SORT>	    
	    <TERM>
	      <xsl:value-of select="term"/>
	    </TERM>	    
	    <xsl:for-each select="ancestor-or-self::*[is-identifiable(.)][1]">
	      <LINK>
      ...
      <dl>
	<xsl:for-each-group select="$index/REF" group-by="TERM">
	  <xsl:sort select="SORT" lang="{$doclang}"/>
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>Process elements
      div|div1|div2|div3|div4|div5|div6</desc>
   </doc>
  <xsl:template match="div|div1|div2|div3|div4|div5|div6">
    ...
    <xsl:choose>
      ...
      <xsl:when test="number($depth) &gt; number($splitLevel) or         @rend='nosplit' or ancestor::TEI/@rend='all' or         ancestor::TEI/@rend='frontpage' or         ancestor::TEI/@rend='nosplit'">
	  <xsl:call-template name="doDivBody">
	    <xsl:with-param name="Depth" select="$depth"/>
	  </xsl:call-template>
      </xsl:when>
      <!-- 2. we are at or above splitting level, 
	   so start a new page  -->
      <xsl:when test="number($depth) &lt;= number($splitLevel) and ancestor::front and $splitFrontmatter='true'">
	<xsl:call-template name="makeDivPage">
	  <xsl:with-param name="depth" select="$depth"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="number($depth) &lt;= number($splitLevel) and ancestor::back and $splitBackmatter='true'">
	<xsl:call-template name="makeDivPage">
	  <xsl:with-param name="depth" select="$depth"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="number($depth) &lt;= number($splitLevel) and ancestor::body">
	<xsl:call-template name="makeDivPage">
	  <xsl:with-param name="depth" select="$depth"/>
	</xsl:call-template>
      </xsl:when>
  ...
  <xsl:template match="docTitle|docAuthor|docImprint|titlePage/titlePart|docDate">
  ...
  <xsl:template match="text">
      <xsl:choose>
         ...
         <xsl:when test="ancestor::group and $splitLevel=0">
	   <xsl:call-template name="makeDivPage">
  ...
  <xsl:template name="addCorpusID">
      <xsl:if test="ancestor-or-self::teiCorpus">
  ...
  <xsl:template name="divContents">
	...
	<xsl:choose>
	  <xsl:when test="parent::*/@rend='multicol'">
	    <td valign="top">
	      <xsl:if test="not($Depth = '')">
		<xsl:element name="h{$Depth + $divOffset}">
		  <xsl:for-each select="head[1]">		
		    <xsl:call-template name="rendToClass">
		      <xsl:with-param name="default"/>
		    </xsl:call-template>
		 </xsl:for-each>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:apply-templates mode="xpath" select="ancestor-or-self::TEI/text">
  ...
  <xsl:template name="makeSidebar">
      <xsl:for-each select="ancestor-or-self::TEI/text/body/list[@type='sidebar']">
  ...
  <xsl:template name="linkListContents">
      ...
      <xsl:for-each select="ancestor-or-self::TEI/text">
      <!-- front matter -->
  ...
  <xsl:template name="mainFrame">
      ...
      <xsl:choose>
        ...
        <xsl:for-each select=" descendant-or-self::TEI/text/*[1]/*[1]">
               ...
               <xsl:if test="$autoToc='true'">
                  <xsl:if test="following-sibling::div/head">
                     ...
                     <ul class="toc">
                        <xsl:apply-templates mode="maketoc" select="following-sibling::div">
	    ...
	    -->
            <xsl:choose>
                     <xsl:when test="ancestor-or-self::TEI/group/text">
                        <xsl:apply-templates mode="xpath" select="ancestor-or-self::TEI/group/text">
                           <xsl:with-param name="xpath" select="$currentID"/>
                        </xsl:apply-templates>
                     </xsl:when>
                     <xsl:otherwise>
  ...
  <xsl:template name="mainTOC">
	...
	<xsl:when
	    test="ancestor-or-self::TEI/text/group and
		  $splitLevel=0">
	  <xsl:for-each
	  ...
	  <xsl:for-each
	      select="ancestor-or-self::TEI/text/group">
	...
	<xsl:when
	    test="ancestor-or-self::TEI/text/group">	  
	  <xsl:for-each
	      select="ancestor-or-self::TEI/text/group/text">
	    <h3>
	      <xsl:number/>
	      <xsl:choose>
		...
		<xsl:when
		    test="front/titlePage//title">
		  <xsl:apply-templates select="front/titlePage//title[1]" mode="plain"/>
		</xsl:when>
	      </xsl:choose>
	    </h3>
	    <xsl:for-each select="front">
	...
	<xsl:otherwise>
	  <xsl:if test="$tocFront">
	    <xsl:for-each select="ancestor-or-self::TEI/text/front">
	      <xsl:call-template name="partTOC">
		<xsl:with-param name="part">front</xsl:with-param>
	      </xsl:call-template>
	    </xsl:for-each>
	  ...
	  <xsl:for-each select="ancestor-or-self::TEI/text/body">
	    <xsl:call-template name="partTOC">
	      <xsl:with-param name="part">body</xsl:with-param>
	    </xsl:call-template>
	  </xsl:for-each>
	  ...
	  <xsl:if test="$tocBack">
	    <xsl:for-each select="ancestor-or-self::TEI/text/back">
	      <xsl:call-template name="partTOC">
		<xsl:with-param name="part">back</xsl:with-param>
	      </xsl:call-template>
	    </xsl:for-each>
  ...
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    ...
    <xsl:template name="groupTOC">
      <xsl:variable name="gDepth"
		    select="count(ancestor::group)"/>
      <ul class="toc toc_group{$gDepth}">	    	   
	<li>
	  <xsl:if test="not($autoHead='true') and not(head or body/head or @n)">
	    <xsl:attribute
	  ...
	  <ul>
		...
		<xsl:for-each select="front">
		  <xsl:if
		      test="titlePage/docTitle/titlePart">
		    <span>
		      <xsl:apply-templates
			  select="titlePage/docTitle/titlePart"
			  mode="plain"/>
		    </span>
		  </xsl:if>
		  <xsl:call-template name="partTOC">
  ...
  <xsl:template name="nextLink">
      ...
      <xsl:choose>
         <xsl:when test="following-sibling::TEI">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::TEI[1]"/>
         </xsl:when>
         ...
         <xsl:when test="parent::body/following-sibling::back/div[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink"
                                 select="parent::body/following-sibling::back/div[1]"/>
         </xsl:when>
         <xsl:when test="parent::front/following-sibling::body/div[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink"
                                 select="parent::front/following-sibling::body/div[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div1' and following-sibling::div1[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::div1[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div2' and following-sibling::div2[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::div2[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div3' and following-sibling::div3[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::div3[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div4' and following-sibling::div4[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::div4[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div5' and following-sibling::div5[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::div5[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div6' and following-sibling::div6[head or $autoHead='true']">
            <xsl:apply-templates mode="generateNextLink" select="following-sibling::div6[1]"/>
         </xsl:when>
      </xsl:choose>
  </xsl:template>
  <doc xmlns="http://www.oxygenxml.com/ns/doc/xsl">
  ...
  <xsl:template name="pageLayoutCSS">
      ...
      <html>
	   ...
	   <xsl:call-template name="javascriptHook"/>
         ...
         <body>
	   <xsl:copy-of select="text/body/@unload"/>
    ...
    <div id="hdr">
      ...
      <xsl:choose>
         ...
         <xsl:when test="@rend='frontpage'">
	           <div class="column-wrapper">
	              <div id="rh-col">
	                 <xsl:for-each select="descendant-or-self::TEI/text/body">
	                    <xsl:apply-templates/>
	                 </xsl:for-each>
	              </div>
	              <div id="lh-col">
	                 <xsl:for-each select="descendant-or-self::TEI/text/front">
	                    <xsl:apply-templates/>
	                 </xsl:for-each>
	              </div>
	           </div>
  ...
  <xsl:template name="col3">
      <div id="col3">
         <xsl:for-each select="descendant-or-self::TEI/text/back">
            <xsl:apply-templates/>
         </xsl:for-each>
      </div>
  </xsl:template>
  ...
  <xsl:template name="pageLayoutSimple">
      <html>
	   ...
	   <xsl:if test="not(text/front/titlePage)">
	    ...
	    <xsl:if test="$autoToc='true' and (descendant::div or descendant::div1) and not(descendant::divGen[@type='toc'])">
	      <h2>
		<xsl:call-template name="i18n">
		  <xsl:with-param name="word">tocWords</xsl:with-param>
		</xsl:call-template>
               ...
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                     <xsl:call-template name="generateTitle"/>: <xsl:choose>
                        ...
                        <xsl:otherwise>
                           <xsl:apply-templates mode="xpath" select="descendant-or-self::TEI/text">
                              <xsl:with-param name="xpath" select="$currentID"/>
                              <xsl:with-param name="action" select="'header'"/>
                           </xsl:apply-templates>
                        </xsl:otherwise>
  ...
  <xsl:template name="previousLink">
      ...
      <xsl:choose>
         <xsl:when test="preceding-sibling::TEI">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::TEI[1]"/>
         </xsl:when>
         ...
         <xsl:when test="parent::body/preceding-sibling::front/div[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink"
                                 select="parent::body/preceding-sibling::front/div[last()]"/>
         </xsl:when>
         <xsl:when test="parent::back/preceding-sibling::body/div[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink"
                                 select="parent::body/preceding-sibling::body/div[last()]"/>
         </xsl:when>
         <xsl:when test="$myName='div1' and preceding-sibling::div1[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::div1[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div2' and preceding-sibling::div2[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::div2[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div3' and preceding-sibling::div3[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::div3[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div4' and preceding-sibling::div4[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::div4[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div5' and preceding-sibling::div5[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::div5[1]"/>
         </xsl:when>
         <xsl:when test="$myName='div6' and preceding-sibling::div6[head or $autoHead='true']">
            <xsl:apply-templates mode="generatePreviousLink" select="preceding-sibling::div6[1]"/>
         </xsl:when>
      </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="tocSection">
      ...
      <xsl:choose>
         ...
         <xsl:when test="div2 and (number($splitLevel) &gt;=1 or $force='true')">
         ...
         <xsl:when test="div3 and (number($splitLevel) &gt;=2 or $force='true')">
         ...
         <xsl:when test="self::div">
            ...
            <xsl:if test="(number($splitLevel)&gt;number($depth)  or $force='true' or ancestor::TEI/@rend='nosplit')">
  ...
  <xsl:template name="xrefpanel">
	...
	<xsl:if test="not(ancestor-or-self::TEI[@rend='nomenu'])">
	  <xsl:call-template name="previousLink"/>
	  <xsl:call-template name="nextLink"/>
	</xsl:if>
      </p>
